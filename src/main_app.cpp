...
//    mainWindow.on_render();
    ...
    ImGui::SetNextWindowSize(ImVec2((float)w, (float)h));// , ImGuiSetCond_FirstUseEver);
    
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
    if (ImGui::Begin("Main Window", &bShowFPS, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
...
void MainApp::numToStr(float num, std::string& out_String)
{
...
void MainApp::ImVec2toString(const ImVec2& num, std::string& out_String)
{

}

void MainApp::OpenImage()
{
    char const * filename;
...
//#include "matFileOutput.h"

bool MainApp::IsReadyForExport(int& out_Result, bool bSilent /*=false*/)
{
...
void MainApp::ExportToClipboard()
{
    ...
    glfwSetClipboardString(window, sstr.str().c_str());
    SetUseIMGUICursor(true);
}

void MainApp::ExportPoints()
{
    SetUseIMGUICursor(false);
...
void MainApp::SortArray(std::vector<ImVec2>& Array)
{
    struct sort_class_x
    {
        ImVec2 origin;
...
void MainApp::UpdateSubdivision(bool bUpdateAll)
{
...
void MainApp::SnapToCurve(ImVec2& ImagePoint)
{
...
void MainApp::SnapToBary(ImVec2& ImagePoint)
{
    int localX, localY;//to this vars local coords will be written
...
ImVec2 MainApp::ConvertImageToReal(const ImVec2& ImagePoint)
{
    ImVec2 RealPoint;
...
bool MainApp::MakeFullLine(ImVec2 Point, ImVec2 Direction, ImVec2& out_Start, ImVec2& out_End, ImVec2 RegionSize, ImVec2 RegionTL/*=ImVec2(0.0f,0.0f)*/)
{
    out_Start = Point;
...
void MainApp::ProcessInput(ImVec2 &HoveredPixel)
{
                    ...
                    switch (CurrentMode)
                    {
                        ...
                        case ActionMode1_AddPoints:
                            if (SelectedItem >= 0)
                            {
                                ...
                                if (bCtrlPressed) // snap only if we hold CTRL
                                {
...
void MainApp::ShowPoints(float im_scale, ImVec2 im_pos, ImVec2 MousePos)
{
    ImVec2 WinPos = ImGui::GetWindowPos();
    ...
    for (size_t kp = 0; kp < UserPoints.size(); kp++)
    {
        ...
        if (CurrentMode == ActionMode1_AddPoints)
        {
            if (kp == SelectedItem || (SelectedItem == -1 && kp == HoveredItem))
            {
                ...
                if (!bCtrlPressed && bShiftPressed && !ImGui::IsMouseDown(0))
                {
...
void MainApp::ShowTickConfigPopup()
{
    ...
    if (ImGui::BeginPopupModal("TickConfig", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::Button("OK", ImVec2(120, 0)) || bEnterReleased)
        {
...
bool MainApp::ShowZoomWindow(const ImVec2 &canvas_sz, const ImVec2 &HoveredPixel, ImVec2& out_ZoomOrigin)
{
    ImVec2 WinPos = ImGui::GetWindowPos();
    ImVec2 MousePos = ImGui::GetMousePos();
...
void MainApp::ShowTickLines(ImVec2 im_pos)
{
    ImVec2 WinPos = ImGui::GetWindowPos();
...
void MainApp::ShowCoordSystem(const ImVec2 &im_pos)
{
    if(!image)
        return;
...
void MainApp::UpdateHoveredItemIndex(ImVec2 im_pos)
{
    ImVec2 WinPos = ImGui::GetWindowPos();
    ImVec2 MousePos = ImGui::GetMousePos();
...
std::string MainApp::unescape(const std::string& s)
{
    std::string res;
...
std::string MainApp::escape(const std::string& s)
{
    std::string res;
    std::string::const_iterator it = s.begin();
    while (it != s.end())
...
void MainApp::ImGui_PopDisableButton()
{
    ImGui::PopStyleColor(3);
}

void MainApp::ImGui_PushDisableButton()
{
